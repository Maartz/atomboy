defmodule Atomboy.Export do
  ...
  Nothing here parses a command line, and nothing here calls `Mix`: a
  Burrito release ships a BEAM and this application, never the build tool,
  and the app's own "Export Movie…" goes through the shipped engine. Both
  doors — `mix atomboy.export` and the executable's `--export` — assemble
  the same `plan/1` out of whatever their arguments look like and hand it to
  `run/1`.
  ...
  Takes the four things an export needs, however its caller came by them:

    * `:tas` — the movie to replay.
    * `:rom` — the cartridge it was recorded on. A movie carries its
      cartridge's title and checksums, never the cartridge itself.
    * `:out` — the file to write; the extension picks the format.
    * `:scale` — an integer nearest-neighbour upscale, or `nil` for the
      format's own default (2 for MP4, 1 for GIF).
  """
  @spec plan(keyword()) :: {:ok, plan()} | {:error, String.t()}
  def plan(opts) do
    tas = Keyword.fetch!(opts, :tas)
    rom_path = Keyword.fetch!(opts, :rom)
    out = Keyword.fetch!(opts, :out)

    with {:ok, format} <- format(out),
         {:ok, scale} <- scale(Keyword.get(opts, :scale), format),
         {:ok, rom} <- cartridge(rom_path),
         {:ok, movie} <- movie(tas),
         :ok <- movie |> Movie.verify(rom) |> mismatch(tas, rom_path),
  ...
  # The Game Boy's panel is 160×144, which is a postage stamp on a modern
  # screen: MP4 defaults to 2, GIF to 1 (where every pixel is paid for twice
  # over).
  defp scale(nil, format), do: {:ok, if(format == :mp4, do: 2, else: 1)}
  defp scale(n, _format) when is_integer(n) and n >= 1, do: {:ok, n}

  defp scale(n, _format),
    do: {:error, "--scale #{n}: a whole number of pixels per pixel, 1 or more."}
  ...
  @doc """
  Replays the plan's movie and encodes it, returning the one line worth
  saying about the file that came out.
  """
  @spec run(plan()) :: {:ok, String.t()} | {:error, String.t()}
  def run(plan) do
    ...
    try do
      ...
      case encode(plan, video, audio, scratch) do
        :ok ->
          {:ok,
  ...
  # of play, and no line drawn faster than an eye can read it. The app's
  # progress sheet reads these very lines off the child's stderr, which is
  # the second reason they are not on stdout: that pipe belongs to the
  # server protocol.
  ...
  # folded onto `acc`. The export writes both into files; the tests count
  ...
  defp ffmpeg(plan, args) do
    ...
    case System.cmd(plan.ffmpeg, args, stderr_to_stdout: true) do
      {_output, 0} -> :ok
      {output, status} -> {:error, "ffmpeg failed (status #{status}):\n#{String.trim(output)}"}
    end
  end
